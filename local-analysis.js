...
// Local script to run the analysis without Netlify
const { chromium } = require('playwright-chromium');
const fs = require('fs').promises;
const path = require('path');

// Import the functions from netlify functions (with some adjustments)
async function scrapeFantasyProsResults(week = '1') {
    ...
    for (const [position, urlPosition] of Object.entries(positions)) {
        ...
        try {
            ...
            await page.waitForTimeout(2000); // Delay between requests
...
async function main() {
    ...
    console.log(`\n🏈 Starting Justin Boone Week ${week} Analysis\n`);
    
    try {
        // Read Justin Boone's predictions
        console.log('📖 Reading Justin Boone predictions...');
        const predictions = JSON.parse(await fs.readFile('./rankings.json', 'utf8'));
        ...
        const resultsFile = `./data/week${week}_actual_results.json`;
        ...
        // Save the analysis
        const analysisFile = `./data/week${week}_analysis.json`;
        ...
        console.log(`\n✅ Analysis complete! Check ${analysisFile} for full details.`);
